...
public class UserAccountController {
	...
	@PostMapping("/bindVirtualWallet")
	@ResponseBody
	public Result bindVirtualWallet(VirtualWalletParam param) {
		UserAccountDetails user = (UserAccountDetails) SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
		param.setUserAccountId(user.getUserAccountId());
		userAccountService.bindVirtualWallet(param);
		return Result.success();
	}

	@GetMapping("/getVirtualWalletInfo")
	@ResponseBody
	public Result getVirtualWalletInfo() {
		UserAccountDetails user = (UserAccountDetails) SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
		return Result.success().setData(userAccountService.getVirtualWalletInfo(user.getUserAccountId()));
	}

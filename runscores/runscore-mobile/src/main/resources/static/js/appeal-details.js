var appealDetailsVM = new Vue({
	...
	mounted : function() {
		...
		if (id != null && id != '') {
			that.showViewDetailsPage(id);
		}
		var orderId = getQueryString('orderId');
		if (orderId != null && orderId != '') {
			that.showViewDetailsPageByOrderId(orderId);
		}
	...
	methods : {
		...
		showViewDetailsPageByOrderId : function(orderId) {
			var that = this;
			that.$http.get('/appeal/findAppealByOrderId', {
				params : {
					orderId : orderId
				}
			}).then(function(res) {
